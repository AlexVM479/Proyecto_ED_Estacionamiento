...
public class HistorialController {
    ...
    public List<HistorialEstacionamiento> buscarHistorialPorCodigo(int codigo) {
        ...
        try (Connection conn = Conexion.conectar()) {
            ...
            Collections.sort(historial, new Comparator<HistorialEstacionamiento>() {
                public int compare(HistorialEstacionamiento h1, HistorialEstacionamiento h2) {
                    int cmp = h1.getFecha().compareTo(h2.getFecha());
                    if (cmp != 0) return cmp;
                    return h1.getHoraEntrada().compareTo(h2.getHoraEntrada());
                }
            });
    ...
    public List<HistorialEstacionamiento> buscarHistorialPorPlaca(String placa) {
        ...
        try (Connection conn = Conexion.conectar()) {
            ...
            // Ordenar manualmente por nombre, fecha y horaEntrada
            Collections.sort(historial, new Comparator<HistorialEstacionamiento>() {
                public int compare(HistorialEstacionamiento h1, HistorialEstacionamiento h2) {
                    int cmp = h1.getNombreConductor().compareToIgnoreCase(h2.getNombreConductor());
                    if (cmp != 0) return cmp;

                    cmp = h1.getFecha().compareTo(h2.getFecha());
                    if (cmp != 0) return cmp;
